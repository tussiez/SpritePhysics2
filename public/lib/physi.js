...
let Physijs = (function () {
  ...
  Physijs.Scene = function (params) {
    ...
    this._worker = new Worker(Physijs.scripts.worker || 'physijs_worker.js', { type: 'module' });
    ...
    this._worker.onmessage = function (event) {
      ...
      if (data instanceof Float32Array) {
        ...
        if (data.cmd) {
          ...
          switch (data.cmd) {
            case 'worker_loaded':
            console.log('Physijs worker loaded')
              params = params || {};
              params.ammo = Physijs.scripts.ammo || 'ammo.js';
              params.fixedTimeStep = params.fixedTimeStep || 1 / 60;
              params.rateLimit = params.rateLimit || true;
              thisIsSomeScope.execute('init', params);
              self.onLoad();
              break;
            case 'objectReady':
              _temp = data.params;
