...
import Ammoo from '/lib/ammo-wasm.js'
let someCrazyFillerVariableName = new Ammoo().then(Ammo => {
var
...
public_functions.removeObject = function( details ) {
	...
	_num_objects--;
  console.log(_objects[details.id])
};
	...
	for ( i = 0; i < num; i++ ) {
		...
		for ( j = 0; j < num_contacts; j++ ) {
			...
			//if ( pt.getDistance() < 0 ) {
				...
				collisionreport[ offset ] =  _objects_ammo[manifold.getBody0().hy].id;
				collisionreport[ offset + 1 ] =_objects_ammo[manifold.getBody1().hy].id;
...
transferableMessage({cmd: 'worker_loaded'}); // ready!
});